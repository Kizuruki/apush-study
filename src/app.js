...
function renderUnit() {
  ...
  // Overview now comes from unit_overviews.json (generated by fetch_fiveable.py)
  // The saved HTML folders are used for the full notes view inside renderQuiz
...
function renderQuiz() {
  ...
  if (state.topicView === 'notes') {
    ...
    // Fiveable: inline fetched HTML in a sandboxed div (works on GitHub Pages)
    const fiveableFolderPath = `notes/fiveable/topic_${topic.id.replace('.', '_')}/index.html`;
    const fiveableContent = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;overflow:hidden;margin-bottom:12px">
        <div style="font-size:10px;letter-spacing:2px;font-weight:700;text-transform:uppercase;color:#059669;padding:12px 16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
          <span>📗 Fiveable Study Guide</span>
          <a href="${fiveableFolderPath}" target="_blank"
             style="font-size:11px;color:#059669;text-decoration:none;font-weight:600;letter-spacing:0">
            Open fullscreen ↗
          </a>
        </div>
        <div id="fiveable-content-${topic.id.replace('.','_')}"
             style="padding:16px;font-size:13px;line-height:1.8;color:var(--navy);min-height:200px">
          <div style="color:var(--muted);text-align:center;padding:40px">Loading study guide…</div>
        </div>
      </div>`;
    ...
    setTimeout(() => loadFiveableContent(topic.id, fiveableFolderPath), 0);
    return `
...
async function loadFiveableContent(topicId, htmlPath, containerId = null) {
  ...
  try {
    ...
    // Remove scripts, nav, footer, header clutter
    doc.querySelectorAll('script, nav, footer, header, aside, [class*="paywall"], [class*="modal"]')
       .forEach(el => el.remove());

    const body = doc.querySelector('main') || doc.querySelector('article') || doc.body;
    container.innerHTML = body ? body.innerHTML : '<p style="color:var(--muted)">Could not parse content.</p>';
    ...
    container.innerHTML = `
      <div style="color:var(--muted);text-align:center;padding:24px">
        ...
        <div style="font-weight:600;margin-bottom:4px">Study guide not saved yet</div>
        <div style="font-size:12px">Save this topic from saveweb2zip.com into<br>
          <code style="background:#f3f4f6;padding:2px 6px;border-radius:4px">notes/fiveable/topic_${topicId.replace('.','_')}/</code>
        </div>
      </div>`;
  }
}
